...
const SpeakerCard = ({ speaker }: { speaker: Speaker }) => {
  return (
    ...
    >
      <div className="h-full bg-gradient-to-br from-[#6715ff] via-[#fd00ff] to-[#6715ff] p-[2px] opacity-60 group-hover:opacity-100 transition-opacity duration-500 rounded-lg">
          ...
          <div className="flex-1 p-5 bg-gradient-to-b from-[#000018] to-[#66147a]/20 flex flex-col justify-between border-t border-[#24ff54]/20">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {speaker.tags.slice(0, 2).map((tag) => (
                <Badge key={`${speaker.id}-${tag}`}>{tag}</Badge>
              ))}
            </div>
          </div>
        </div>
